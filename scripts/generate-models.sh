...
quicktype --src-lang schema --lang py \
  -o "$PROJECT_ROOT/python-server/src/domain/user/entities/user_aggregates_schema.py" \
  --python-version 3.7 --nice-property-names \
  "$SCHEMAS_DIR/UserAggregates.json"

quicktype --src-lang schema --lang py \
  -o "$PROJECT_ROOT/python-server/src/domain/user/entities/profile_entity_schema.py" \
  --python-version 3.7 --nice-property-names \
  "$SCHEMAS_DIR/ProfileEntity.json"

quicktype --src-lang schema --lang py \
  -o "$PROJECT_ROOT/python-server/src/domain/user/entities/inventory_entity_schema.py" \
  --python-version 3.7 --nice-property-names \
...
quicktype --src-lang schema --lang ts \
  -o "$PROJECT_ROOT/nodejs-server/src/domain/user/entities/UserAggregatesSchema.ts" \
  --nice-property-names \
  "$SCHEMAS_DIR/UserAggregates.json"

quicktype --src-lang schema --lang ts \
  -o "$PROJECT_ROOT/nodejs-server/src/domain/user/entities/ProfileEntitySchema.ts" \
  --nice-property-names \
  "$SCHEMAS_DIR/ProfileEntity.json"

quicktype --src-lang schema --lang ts \
  -o "$PROJECT_ROOT/nodejs-server/src/domain/user/entities/InventoryEntitySchema.ts" \
  --nice-property-names \
  "$SCHEMAS_DIR/InventoryEntity.json"
...
quicktype --src-lang schema --lang go \
  -o "$PROJECT_ROOT/golang-server/src/domain/user/entities/user_aggregates_schema.go" \
  --package entities \
  "$SCHEMAS_DIR/UserAggregates.json"

quicktype --src-lang schema --lang go \
  -o "$PROJECT_ROOT/golang-server/src/domain/user/entities/profile_entity_schema.go" \
  --package entities \
  "$SCHEMAS_DIR/ProfileEntity.json"

quicktype --src-lang schema --lang go \
  -o "$PROJECT_ROOT/golang-server/src/domain/user/entities/inventory_entity_schema.go" \
  --package entities \
  "$SCHEMAS_DIR/InventoryEntity.json"
...
quicktype --src-lang schema --lang cs \
  -o "$PROJECT_ROOT/csharp-server/src/Domain/User/Entities/UserAggregatesSchema.cs" \
  --namespace HandInHand.Domain.User.Entities \
  "$SCHEMAS_DIR/UserAggregates.json"

quicktype --src-lang schema --lang cs \
  -o "$PROJECT_ROOT/csharp-server/src/Domain/User/Entities/ProfileEntitySchema.cs" \
  --namespace HandInHand.Domain.User.Entities \
  "$SCHEMAS_DIR/ProfileEntity.json"

quicktype --src-lang schema --lang cs \
  -o "$PROJECT_ROOT/csharp-server/src/Domain/User/Entities/InventoryEntitySchema.cs" \
  --namespace HandInHand.Domain.User.Entities \
  "$SCHEMAS_DIR/InventoryEntity.json"
...
echo "📂 생성된 파일 위치:"
echo "   🐍 Python: python-server/src/domain/user/entities/"
echo "   🟨 Node.js: nodejs-server/src/domain/user/entities/"
echo "   🔵 Go: golang-server/src/domain/user/entities/"
echo "   🟣 C#: csharp-server/src/Domain/User/Entities/"
echo ""
echo "📋 다음 단계:"
